...
def get_model():
    ...
    num_hidden_input = 250
    nb_dense_layers = [50,200,450, 100,50, 400]
...
def get_subject_preproc(subject,samplesize="83Perc"):
    foldername = "../0 Data/DB3/DB3_preproc_Features/{0}/{1}/".format(str(subject),samplesize)
...
with open("results_DB3_features.csv","w") as wf:
...
for subject in range(1,12):
    ...
    #save model
    foldername = "./models/DB3_features/model{}/".format(str(subject))
    os.mkdir(foldername) 
    ...
    for retraining_samples in ["83Perc","60Perc", "30Perc","16Perc","fiveMovements","oneMovement"]:
        ...
        for retraining_method in ["all","first", "last","first_last","new"]:    
            ...
            with open("results_DB3_features.csv","a") as wf:
