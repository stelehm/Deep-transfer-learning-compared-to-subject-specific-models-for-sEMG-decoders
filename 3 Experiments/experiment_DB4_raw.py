...
def get_model():
    ...
    model.add(tf.keras.layers.Dense(12, kernel_regularizer="l2", activation="softmax"))
    
    return model


def get_subject_preproc(subject,samplesize="83Perc"):
    foldername = "../0 Data/DB4/DB4_preproc_raw/{0}/{1}/".format(str(subject),samplesize)
...
with open("results_DB4_raw.csv","w") as wf:
...
for subject in range(1,11):
    ...
    #save model
    foldername = "./models/DB4_raw/model{}/".format(str(subject))
    os.mkdir(foldername) 
    ...
    for retraining_samples in ["83Perc","60Perc", "30Perc","16Perc","fiveMovements","oneMovement"]:
        ...
        csv_logger_new = tf.keras.callbacks.CSVLogger("model{}_samples{}_newModel_history_log.csv".format(str(subject),retraining_samples), append=True)
        #train
        ...
        for retraining_method in ["all","first", "last","first_last","new"]:    
            ...
            if retraining_method == "first":
                ...
                for layer in retrain_model.layers:
                    layer.trainable = False
                    retrain_model.add(tf.keras.layers.Dense(12, kernel_regularizer="l2", activation="softmax"))
            ...
            csv_logger_retrain = tf.keras.callbacks.CSVLogger("model{}_samples_{}_method_{}_retrain_history_log.csv".format(str(subject),retraining_samples,retraining_method), append=True)
            ...
            with open("results_DB4_raw.csv","a") as wf:
